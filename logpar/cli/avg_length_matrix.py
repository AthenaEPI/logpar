...
def avg_length_matrix(streamlines_file, labeled_volume_file, labels_file, outfile):
    ...
    for i in xrange(nlabels):
        for j in xrange(i+1, nlabels):
            ...
            avg_len = numpy.mean([streamline_utils.length(s)
                                  for s in grouping[mi, mj]])
            conn_matrix[j, i] = conn_matrix[i, j] = avg_len
